...
model { 
  ...
  for (i in 1:N) {
  target += multi_normal_lpdf(to_vector({M[i],iEL[i]}) | mus[{1,4}], Sigma[{1,4},{1,4}]); //OK
  target += multi_normal_lpdf(to_vector({Yl[i],M[i]})  | mus[{2,3}], Sigma[{2,3},{2,3}]); //OK
  target += multi_normal_lpdf(to_vector({Yl[i],iEL[i]})| mus[{2,4}], Sigma[{2,4},{2,4}]); //OK
  }
} 
