...
model { 
  ...
  for (i in 1:N) {
  target += multi_normal_lpdf(to_vector({E[i],M[i]}) | mus[{1,3}], Sigma[{1,3},{1,3}]); 
  target += multi_normal_lpdf(to_vector({Yl[i],M[i]})  | mus[{2,3}], Sigma[{2,3},{2,3}]); 
  target += multi_normal_lpdf(to_vector({Yl[i],iEL[i]})| mus[{2,4}], Sigma[{2,4},{2,4}]); 
  }
} 
