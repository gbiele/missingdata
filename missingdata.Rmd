...
We do this by first getting an overview about type of missing data, then moving on to general ways to deal with missing data, and finally looking at a few examples to do analyses in R when data is missing.
...
When using a function on a object with missing data, R returns `NA`.
...
To represent missingness, m-graphs introduce a new variable `O*` for observable O's. `O*` depends on O and R~O~, the latter of which indexes id O is observable. At the same time, the original variable O becomes a unobserved (latent) variable. Here is such a graph:
...
Next, a DAG that described missingness at random (MAR):
...
Here, missingness is random, given that we know education.

Finally, we can look at a situation that where missingness is not at random, even taking into account age:
...
This method is not perfect, because it still results in inflated variances of parameter estimates. Still, this method should not be dismissed, because it allows a relatively easy way to obtain consistent estimates, given a specific MAR structure.
...
As expected, the standard error for the adjusted model based on complete data is larger (less data, more predictors).
...
The correct total effect of E on O is `r round(es[1,1],digits = 2)`. To recover this, we fit a FIML regression that models the effect of E and O and, importantly, the co-variances for E, O, and M:
...
An alternative to the FIML approach is to use multiple imputation by chained equation. The most popular R package to implement this is [mice](https://cran.rstudio.com/web/packages/mice/index.html). Some people seem to have [strong opinions about FIML vs multiple imputations](https://statisticalhorizons.com/ml-better-than-mi). A fair summary seems to be that
* FIML is preferred when missingness does not depend on auxiliary variables, the association between the involved variables is well described by a multivariate normal distribution, and there are no strong non-linearity (in the associations between variables)
...
Note that the lower mean for the imputed data is expected, because our data generating model specified that cases with low exposure values were more likely to having missing data for the outcome variable.
...
plot(my_data$O[idx_missing],
     ...
     ylab = "imputed")
```
It did (Unfortunately we can't do this for real data. But we can do simulations to check if imputation is possible given an assumed DAG and percentage of missing data)
...
The object MI_fit is of the class [mira](https://stefvanbuuren.name/mice/reference/mira-class.html), and contains in slots 4 for each imputed data set the result of the regression model we specified. We can look at all coefficients:
...
In all previous analyses we could use `dagittie`'s `simulateSEM` function to generate data from a DAG, because we treated data as multivariate normal. However, we can't do this now, because
...
Note that here we transformed the ordinal variable L to a number variable, because `lavaan` cannot deal with categorical variables when doing FIML estimation (Mplus can). Still, the result are off, because we haven't added information about the interaction to the model. The next model does this.
...
Indeed, the result look nice. This result drives again home that it is important to have a good idea about the data generating process when one deals with missing dat.

Now let's try multiple imputation by chained equations (MICE).
...
While the imputation was not perfect (there is a problem with L1) we see that we have filled in lost of cases for groups L1-L3, where the correlation is typically positive. A complete cases analysis without these imputed data points naturally underestimates the association between E and O. Still, before we move on, lets see if we can improve the imputation by using a different imputation method than predictive mean matching (pmm) for Ox.
...
OK, this looks much better. The problem with predictive mean matching is, that it will not allow more extreme values than the observed ones. However, for the data generating process we are looking at, the most extreme values were missing. If this is a possibility, pmm is not the best imputation method.
...
In the current example both FIML and MI allows us to handle missing data properly. Still, MI is generally more flexible because 
...
* Complete cases analysis under missingness in predictor variables results in larger than necessary standard errors
...
* valid ways to deal with missing data include 
    ...
    * Weighting! We haven't talked about this, but inverse probability of "participation" weighting (e.g. in the package `iptw`) can also be used to deal with missing data.
