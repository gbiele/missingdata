...
An alternative to the FIML approach is to use multiple imputation by chained equation. The most popular r packer to implement this is [mice](https://cran.rstudio.com/web/packages/mice/index.html). Some people seem to have [strong opinions about FIML vs multiple imputations](https://statisticalhorizons.com/ml-better-than-mi). A fair summary seems to be that
...
One good reason to use `mice` is the extensive documentation. Among other things there is an online [book](https://stefvanbuuren.name/fimd/) with lots of useful explanations and example code also for more advanced analyses, and a set of [vignettes](https://www.gerkovink.com/miceVignettes/) that serve as a moderately long introduction.
...
Note that the lower mean for the imputed data is expected, because our data generating model specified that cases with low exposure values were more likely to miss the outcome variable.
