...
```{r mice_plots, fig.width=4, fig.height=4}
par(mfrow = c(2,2))
...
* C is a categorical variables and
* the effect of C on O is assumed to be that of an effect modifier.
...
OK, this doesn't work (you can make it work in MPLUS). Let's just make a numeric variable Ln as a work-around:

```{r FIMLi_fit_workaround}
mar_interaction_data$Ln1 = scale(as.numeric(mar_interaction_data$L))
mar_interaction_data$Ln2 = mar_interaction_data$Ln1^2
...
So, the correct regression coeficient is `r round(coef(lm(O ~ E))[2],2)` and with the FIML model we get `r round(coef(FIMLi_fit)[1],2)`. How about fit measures for this model?

```{r FIMLi_fit_fitmeasures}
FM = fitMeasures(FIMLi_fit)
FM[grep("tli|cfi|rmsea", names(FM))]
```

OK, we have a model that is totally garbage, and yet has totally fine fit indices. I hope this reinforces how important it is to carfully think about the data at hand, before fitting models (I whish I had understood this).


Now lets try multiple imputation. To make things more realistic, we add a bit more missing data (completely random).

```{r mice2}
idata = mar_interaction_data[,c("E","M","L","Ox")]
for (v in 1:3)
...
This model does not take into account interaction terms. `mice` uses the concept of _passive imputation_ to insure that erived variables (like interactions) are consistent with imputations throughout the imputation process. Detailed information about specifyingh such more complex imputation models can be found [here](https://stefvanbuuren.name/fimd/sec-knowledge.html)

```{r mice2_add_method}
idata$iEL = idata$E * idata$M
...
And we plot the resulting imputations
...
While the imputation was not perfect (thre is a problem with L1) we see that we have filled in lost of cases for groups L1-L3, where the correlation is typically positive. A complete cases analysis Without these imputed data points naturally underestimates the association between E and O.
...
The estimate from imputed data is not as good as we 
